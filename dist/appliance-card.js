  ...
  _render() {
    ...
    this.innerHTML = `
      <div style="padding: 10px; font-family: sans-serif; background: #1c1c1c; color: white;">
        ...
        <input id="main-ent" type="text" style="width:100%; padding:10px; margin:8px 0; background:#000; color:#fff; border:1px solid #444;" 
        ...
        <div style="display: flex; gap: 5px; margin-top:8px;">
          <input id="new-sensor" type="text" style="flex: 1; padding: 10px; background:#000; color:#fff; border:1px solid #444;" placeholder="sensor.puissance">
          <button id="add-btn" style="padding: 0 15px; background: #7CFFB2; border: none; font-weight: bold;">+</button>
        </div>
          ...
          ${((this._config.sensors && this._config.sensors[type]) || []).map((s, i) => `
            <div style="background: #333; padding: 8px; border-radius: 4px; border: 1px solid #444;">
              <div style="display:flex; justify-content:space-between; margin-bottom:5px;">
                ...
                <button class="remove-btn" data-index="${i}" style="background:none; color:#ff5252; border:none; cursor:pointer; font-weight:bold;">Supprimer</button>
              </div>
              <input class="name-edit" data-index="${i}" type="text" style="width:100%; background:#222; color:#fff; border:1px solid #555; padding:5px; font-size:11px;" 
    ...
    this.querySelector('#add-btn').onclick = () => {
      ...
      if (input.value.trim()) {
        ...
        typeSensors.push({ entity: input.value.trim(), name: "" });
...
class ApplianceCard extends HTMLElement {
  ...
  set hass(hass) {
    ...
    const map = { 'wash':'lavage', 'rinse':'rincage', 'spin':'essorage', 'finish':'findecycle', 'off':'enveille', 'none':'enveille' };
    ...
    this.querySelector('#state-text').textContent = stateObj ? stateObj.state : 'OFF';
    ...
    sensors.forEach(s => {
      ...
      if (state) {
        ...
        const title = s.name || s.entity.split('.').pop().replace('_',' ');
        div.innerHTML = `
          <div style="font-size: 8px; opacity: 0.6; text-transform: uppercase;">${title}</div>
          <div style="font-size: 11px; font-weight: bold;">${state.state} ${state.attributes.unit_of_measurement || ''}</div>
        `;
